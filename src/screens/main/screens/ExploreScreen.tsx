...
const ExploreScreen = () => {
  ...
  const onSearch = (searchText: string) => {
    var filtered = productList.filter(item =>   item.name.toLowerCase().includes(searchText.toLowerCase()))
  ...
  testProduct = new ProductModel(2,'Bomb', 75, 5, 'A bouquet', 2.4, 1, [
    'https://images.pexels.com/photos/953057/pexels-photo-953057.jpeg',
  ]);
  productList.push(testProduct);
  testProduct = new ProductModel(3,'Rot', 13, 5, 'A bouquet', 2.4, 1, [
    'https://images.pexels.com/photos/2099737/pexels-photo-2099737.jpeg',
  ]);
  productList.push(testProduct);
  testProduct = new ProductModel(4,'Roto', 36, 5, 'A bouquet', 2.4, 1, [
    'https://images.pexels.com/photos/250716/pexels-photo-250716.jpeg',
  ]);
  productList.push(testProduct);
  testProduct = new ProductModel(5,'Comb', 78, 5, 'A bouquet', 2.4, 1, [
    'https://images.pexels.com/photos/372166/pexels-photo-372166.jpeg',
  ]);
  productList.push(testProduct);
  testProduct = new ProductModel(6,'Sap', 54, 5, 'A bouquet', 2.4, 1, [
  ...
  return (
    <View style={styles.view}>
      <CustomInput onChangeText={(text) => onSearch(text)} icon={faSearch} marginBottom={12}/>
      ...
      <CustomButton onPressed={onDisplayPressed}>
        <ItemRow justifyContent="flex-start">
          ...
          <CustomText type='subTitle'>{isColumn ? "Colm" : "Grid"}</CustomText>
        </ItemRow>
        </CustomButton>
      </View>
      {isColumn ==false ? (
